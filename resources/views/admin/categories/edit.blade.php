                    ...
                    <div class="tab-content mt-3" id="languageTabContent">
                        @foreach($activeLanguages as $language)
                            ...
                            <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}" 
                                ...
                                @if($loop->first)
                                <!-- Category Image (Only show for first language tab) -->
                                <div class="mb-3">
                                    <label for="image" class="form-label">{{ __('cms.categories.image') }}</label>
                                    @if($category->image)
                                        <div class="mb-2">
                                            <img src="{{ asset('images/' . $category->image) }}" alt="{{ $category->translation->name ?? 'Category Image' }}" class="img-thumbnail" style="max-width: 200px;">
                                        </div>
                                    @endif
                                    <input type="file" class="form-control" id="image" name="image">
                                    <small class="text-muted">{{ __('cms.categories.image_help') }}</small>
                                </div>
                                @endif
                                <input type="text" 
